...
export default class MoveCrab {
	...
	static do(legs, chain, data) {
		...
		angles = legs.rightFront.moveLeg(90 + (data.x * 10), y, -40);
		...
		angles = legs.rightBack.moveLeg(90 + (data.x * 10), y, -40);
		...
		angles = legs.rightFront.moveLeg(90 - (data.x * 10), -y, -70);
		chain.load('rfs', angles.shoulder, 0, 200);
		...
		chain.load('rmf', angles.foot, 0, 200);
		angles = legs.rightBack.moveLeg(90 - (data.x * 10), -y, -70);
		chain.load('rbs', angles.shoulder, 0, 200);
